...
//     try {
//       await emailSignup(firstName, lastName, email, password);
//       // navigate("/login"); // 👈 go to terms immediately
//     } catch (err) {
//       console.error(err);
//       setError("Signup failed. Try again.");
//     }
...
//           onClick={handleGoogleSignup}
//           disabled={loading}
//           className="mt-6 w-full flex items-center justify-center bg-white border border-gray-400 py-2 rounded-lg shadow-sm hover:bg-[#B7D4E9]/20 disabled:opacity-50"
//         >
//           <FcGoogle className="h-6 w-6 mr-3" /> Continue with Google
//         </button>
...
//       );
//       e.target.reset(); // only reset after success
//     } catch (err) {
//       console.error(err);
...
//       // Save in localStorage for AdminRoute.jsx
//       localStorage.setItem("userData", JSON.stringify(userData));

//       console.log("User Signed Up with Google:", userData);
//       navigate("/terms"); // redirect to terms first
//     } catch (err) {
//       console.error(err);
//       setError("Google Sign-Up failed. Please try again.");
//     } finally {
...
const Signup = () => {
  ...
  const handleNormalSignup = async (e) => {
    ...
    try {
      ...
      // ✅ Stay on signup page, just show success message
      setSuccess(
        "✅ Verification email sent! Please check your inbox before logging in."
      );
      e.target.reset();
