...
const ProtectedRoute = ({ children }) => {
  ...
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged(async (firebaseUser) => {
      if (firebaseUser) {
        try {
          // 🔥 Force refresh claims after reload
          const tokenResult = await firebaseUser.getIdTokenResult(true);

          // check terms from Firestore
          const snap = await getDoc(doc(db, "Users", firebaseUser.uid));
          const acceptedTerms = snap.exists()
            ? snap.data().acceptedTerms || false
            : false;

          setUser({
            ...firebaseUser,
            isAdmin: !!tokenResult.claims.admin, // ✅ always correct
          });
